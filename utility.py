...
def streamingHDFSToBronze(tableName, schema):
    ...
    dfStreaming = spark.readStream.option("sep", ",").option("header", "true").options(name = "hotinbk_10").schema(streamingSchema).csv("/streaming/" + tableName).withColumn('Date_Time', current_timestamp())
...
def streamingBronzeToGoldAppendMethod(tableName, schema, partitionedBy = []):
    ...
    dfStreaming = spark.readStream.format("delta").options(name = "hotinbk_2").load("/medical/bronze/" + tableName)
