...
def gold_analyze_patients_by_age():
    res = spark.sql("""
    ...
    order by num desc
    """)
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_patients_by_age")

def gold_analyze_admissions_and_deied_patients_in_hospital():
    res = spark.sql("""
    ...
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_admissions_and_deied_patients_in_hospital")

def gold_analyze_get_5_common_diseases_by_month():
    res = spark.sql("""
    select * from 
    ...
    where ROW_NUMBER < 6""")
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_get_5_common_diseases_by_month")

def gold_analyze_diseases_affect_stay_days():
    res = spark.sql("""
    ...
    limit 20
    """)
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_diseases_affect_stay_days")

def gold_analyze_20_common_diseases_clinical_results():
    res = spark.sql("""
    ...
    LIMIT 20
    """)
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_20_common_diseases_clinical_results")

def gold_analyze_state_affect_total_died_patients():
    ...
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_state_affect_total_died_patients")

def gold_analyze_patients_by_sex():
    res = spark.sql("""
    select CASE 
    ...
    order by numCases
    """)
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_patients_by_sex")

def gold_analyze_patients_died_in_hospital():
    ...
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_patients_died_in_hospital")

def gold_analyze_diseases_clinical_affected_died_patients():
    res = spark.sql("""
    ...
    """)
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_diseases_clinical_affected_died_patients")

def gold_analyze_diseases_clinical_by_month():
    res = spark.sql("""
    ...
    where ROW_NUMBER < 21""")
    res.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save("/medical/gold/gold_analyze_diseases_clinical_by_month")